...
const AppContent = () => {
  ...
  const [selectedDashboard, setSelectedDashboard] = useState<'comercial' | 'interno' | 'gdash' | null>(() => {
    const saved = localStorage.getItem('app_selectedDashboard');
    return (saved as any) || null;
  });

  useEffect(() => {
    if (selectedDashboard) {
      localStorage.setItem('app_selectedDashboard', selectedDashboard);
    } else {
      localStorage.removeItem('app_selectedDashboard');
    }
  }, [selectedDashboard]);
